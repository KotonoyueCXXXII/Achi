...
- **建筑保护** — 权限分级（访客 / 成员 / 会员 / 管理员），拦截越权破坏和放置
...
全文统一使用以下术语形式，括号内为等价别名（不再混用）。

| 标准术语 | 别名（不推荐） | 说明 |
|---|---|---|
| 目标选择器（selector） | target selector | `@a`、`@p`、`@s` 等 |
| 循环（for loop） | For 循环语句、Loop | `for ... rof` 代码块 |
| 条件语句（if） | If 语句、分支语句 | `if/elif/else/fi` 代码块 |
| 表单（form） | UI 表单 | 长表单/模态表单/弹窗 |
| 指令（command） | mcfunction 指令 | Minecraft 内置命令 |
| 事件（event） | 系统事件、服务器事件 | `systemevent listen` 监听 |
| 指针（object.ref） | pointer | HPL 对象引用句柄 |
| 域（domain） | ExtraData 域 | `类型.模块.属性` 三段式命名 |
| 槽位（slot） | slot、索引 | 0-indexed 位置编号 |
| 地皮（land） | 领地、Land | 玩家拥有的建筑区域 |
| pin（钉住） | pinned | 防止指针被 GC 回收 |
| finalise（释放） | 释放指针 | 释放 pin 的指针 |
| 哨兵值（sentinel） | 无效值 | `-2147483648` 表示不存在/失败 |
