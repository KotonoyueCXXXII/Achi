...
| `queue/push` | — | 槽位索引 | 否 |
| `queue/pop` | — | — | 否 |
| `queue/peek` | — | pinned 指针或 `-2147483648` | **是** |
...
**读：** `block_destroy.hpl`, `player_leave.hpl`, `item_use.hpl`, `player_join.hpl`, `land/getFreeIndex.hpl`, `HOME/_.hpl`, `HOME/LAND/_.hpl`, `GET.hpl`, `ACTION.hpl`, `DEL.hpl`
...
**写：** `entry/showError.hpl`（1–17），`ACTION.hpl`（17），`entry/navigate.hpl`（18）
